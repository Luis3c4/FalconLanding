...
// `tone` picks light (white, for dark backgrounds like the merged nav pill
// that crosses dark sections) or dark (for the standalone dock, which only
// ever sits over the Hero's light video and needs dark icons to read).
export function SocialIcons({
    ...
    const linkClass = cn(
        ...
        isDark
            ? "text-neutral-800 hover:bg-black/10 hover:text-black"
            : "text-white/90 hover:bg-white/15 hover:text-white",
        iconClassName,
    );
    const iconClass = cn(
        "h-5 w-5",
        isDark ? "drop-shadow-[0_1px_2px_rgba(255,255,255,0.6)]" : "drop-shadow-[0_1px_3px_rgba(0,0,0,0.45)]",
    );
    ...
    return (
        <>
            ...
            <a href="#" className={linkClass}>
                <svg className={iconClass} fill="currentColor" viewBox="0 0 448 512" xmlns="http://www.w3.org/2000/svg">
        ...
        >
            <div className="animate-fade-up flex items-center justify-end" style={{ animationDelay: "240ms" }}>
                <div className="glass-dock flex flex-col gap-3 rounded-full px-3 py-4">
                    <SocialIcons iconClassName="h-10 w-10" tone="dark" />
                </div>
            </div>
        </div>
    );
